...
<div class="container mx-auto my-8">
    <div class="flex flex-col md:flex-row bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div class="md:w-1/3 p-4">
            <img src="{{ $manga->image_url }}" alt="{{ $manga->title }}" class="w-full h-auto object-cover rounded-lg">
        </div>

        <!-- Información del producto -->
        <div class="md:w-2/3 p-6">
            <h2 class="text-3xl font-bold mb-4 text-gray-800">{{ $manga->title }}</h2>
            <p class="text-gray-600 text-lg mb-4">{{ $manga->description }}</p>
            
            <div class="flex items-center mb-4">
                <span class="text-2xl font-bold text-blue-600">${{ $manga->price }}</span>
            </div>
